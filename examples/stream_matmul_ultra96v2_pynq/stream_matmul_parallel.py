...
def mkLed():
    ...
    addrwidth = 4
    par = 4
    ram_a = vthread.RAM(m, 'ram_a', clk, rst, datawidth*par, addrwidth)# 2^addrwidth * datawidth
    ram_b = vthread.RAM(m, 'ram_b', clk, rst, datawidth*par, addrwidth)
    ...
    a = strm.source('a', datawidth=datawidth * par)
    b = strm.source('b', datawidth=datawidth * par)
    ...
    #         if i%(2*s)==0:
    #             c_list[i] = c_list[i]+c_list[i+s]
    #     s *= 2
    sum, sum_valid = strm.ReduceAddValid(c_list[0], size)
...
# 16*(16+16*(16+pipline_depth+5)+16) =  11008
    ...
    def comp(matrix_size, a_offset, b_offset, c_offset):
        ...
        for i in range(matrix_size):
            ...
            for j in range(matrix_size): # matrix_size*(matrix_size+one_operaton_cycle)
                ...
                strm_madd(matrix_size, j) # one_operaton_cycle = (pipline_depth + matrix_size+5)
