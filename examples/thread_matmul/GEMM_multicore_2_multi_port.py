...
def mkLed():
    ...
    ram_a0 = vthread.MultibankRAM(m, 'ram_a0', clk, rst, datawidth, addrwidth, numbanks=para, numports=core_num)# 2^addrwidth * datawidth
    ram_a1 = vthread.MultibankRAM(m, 'ram_a1', clk, rst, datawidth, addrwidth, numbanks=para, numports=core_num)
    ram_a2 = vthread.MultibankRAM(m, 'ram_a2', clk, rst, datawidth, addrwidth, numbanks=para, numports=core_num)
    ram_a3 = vthread.MultibankRAM(m, 'ram_a3', clk, rst, datawidth, addrwidth, numbanks=para, numports=core_num)

    ram_b0 = vthread.MultibankRAM(m, 'ram_b0', clk, rst, datawidth, addrwidth, numbanks=para, numports=core_num)
    ram_b1 = vthread.MultibankRAM(m, 'ram_b1', clk, rst, datawidth, addrwidth, numbanks=para, numports=core_num)
    ram_b2 = vthread.MultibankRAM(m, 'ram_b2', clk, rst, datawidth, addrwidth, numbanks=para, numports=core_num)
    ram_b3 = vthread.MultibankRAM(m, 'ram_b3', clk, rst, datawidth, addrwidth, numbanks=para, numports=core_num)
    ...
    ram_a_address_lists = [[0, 0], [1, 1]]
    ram_b_address_lists = [[0, 1], [0, 1]]
    ...
    def comp_stream(size, offset, indexb):
        _size = size // (core_num*para)
        print("begin")
        print(indexb)
        print("end")
        strm.set_parameter('size', _size)
        strm.set_multicore_port('a', ram_a_address_lists, 'indexa', indexb*_size+offset, _size)
        # strm.set_multicore('a', ram_a_list, offset, _size)
        strm.set_multicore_port('b', ram_b_address_lists, 'indexb', offset, _size)
        ...
        strm.set_sink_parallel('sum_list', ram_c, offset=offset+indexb*_size, size =_size, stride=1)
        strm.run()
        strm.join()
    ...
    def matmul():
        while True:
            ...
            maxi.dma_read_packed(ram_a1, offset, a_offset+matrix_size*4, matrix_size)
    ...
    def comp(matrix_size_a, matrix_size_b, matrix_size_k, c_offset):
        for i in range(matrix_size_a):
            for j in range(matrix_size_b//core_num):
                comp_stream(matrix_size*core_num,  0, i)
