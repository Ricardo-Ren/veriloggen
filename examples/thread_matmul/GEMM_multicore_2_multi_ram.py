...
def mkLed():
    ...
    ram_a_list = [ram_a0, ram_a1]
    ram_b_list = [ram_b0, ram_b1]
    ...
    def comp_stream(size, offset, indexa, indexb):
        ...
        # size 表示多少个循环，一次需要导入para个数字
        strm.set_sink_parallel('sum_list', ram_c, offset=offset+(indexa+indexb)*_size, size =_size, stride=1)
        strm.run()
        strm.join()
...
# actually: 16556 cycle
    def comp(matrix_size_a, matrix_size_b, matrix_size_k, c_offset):
        ...
        maxi.dma_read_packed(ram_b1, 0, b_offset+matrix_size*4, matrix_size)
