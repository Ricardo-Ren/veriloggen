...
from veriloggen.types.ram import RAMInterface


class ExtRAM(vthread.RAM):
    """ Interface-only RAM """
    ...
    def __init__(self, m, name, clk, rst,
        ...
        for i in range(numports):
            interface = RAMInterface(m, name + '_%d' % i, datawidth, addrwidth,
                                     itype='Output', otype='Input', with_enable=True)
...
def mkTarget():
    ...
    ram = ExtRAM(m, 'ram', clk, rst, datawidth, addrwidth)
    def matmul():
        while True:
            wait()
    ...
    def comp(matrix_size, a_offset, b_offset, c_offset):
        ...
        for i in range(matrix_size):
            b_addr = b_offset
            a1 = ram.read(a_addr + 0)
            a2 = ram.read(a_addr + 1)
            a3 = ram.read(a_addr + 2)
            a4 = ram.read(a_addr + 3)
            a5 = ram.read(a_addr + 4)
            a6 = ram.read(a_addr + 5)
            a7 = ram.read(a_addr + 6)   
            a8 = ram.read(a_addr + 7)
            for j in range(matrix_size):
                b1 = ram.read(b_addr + 0)
                b2 = ram.read(b_addr + 1)
                b3 = ram.read(b_addr + 2)
                b4 = ram.read(b_addr + 3)
                b5 = ram.read(b_addr + 4)
                b6 = ram.read(b_addr + 5)
                b7 = ram.read(b_addr + 6)
                b8 = ram.read(b_addr + 7)
...
def mkTest(memimg_name=None):
    ...
    mem = np.zeros([1024 * 8 // datawidth], dtype=np.int64)
...
rslt = run(filename='user_module_all.v')
print(rslt)