...
class Stream(BaseStream):
    ...
    def source_parallel(self, name=None, datawidth=None, point=0, signed=True, no_ctrl=False, par=1):
        v_list = []
        for i in range(par):
            v = strm.source(name + ('_%d' % i))
            v_list.append(v)
        self.parallel_source[name] = (tuple(v_list), par)
    ...
    def set_source_parallel(self, fsm, name, ram, offset, size, stride=1, port=0):
        v_list, par = self.parallel_source[name]
        _size = size // par # note that residue and masking must be considered here
        for v, ram in zip(v_list, ram.rams):
            strm.set_source(v.name, ram, offset, _size)
