...
def mkLed():
    ...
    def comp_stream(size, waddr, offset, indexb):
        ...
        strm.set_sink_parallel('sum_list', ram_c, offset=waddr+indexb*_size, size =_size)
        strm.run()
        strm.join()
    ...
    def matmul():
        while True:
            ...
            maxi.dma_read_packed(ram_a2, offset, a_offset, matrix_size)
    ...
    def comp(matrix_size_a, matrix_size_b, matrix_size_k, c_offset):
        for i in range(matrix_size_a):
            for j in range(matrix_size_b//core_num):
                comp_stream(matrix_size*core_num, c_offset, i, j)
