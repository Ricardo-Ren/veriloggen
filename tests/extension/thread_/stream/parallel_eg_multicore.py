...
def mkLed():
    ...
    def comp_stream(size, offset):
        start_time = counter
        _size = size//2
        strm.set_parameter('size', _size//para)
        # strm.set_source_parallel('a1', ram_a, offset, size//para)
        strm.set_source_parallel_pattern('a1', ram_a, offset, (size, para)) #how to set start address
        # strm.set_source_parallel('a2', ram_a, offset, size//para)
        # strm.set_source_parallel_pattern('a2', ram_a, offset+para//2, (size, para))
        print("ssss:",size//para)
        strm.set_source_parallel_pattern('b1', ram_b, offset, (size, para))
        # strm.set_source_parallel('b2', ram_b, offset, size//para)
        # strm.set_source_parallel_pattern('b2', ram_a, offset+para//2, (size, para))
        #strm.set_source_parallel(name='a', ram = ram_a, offset = offset, size = size//para)
        strm.set_sink('sum1', ram_c, offset, 1)
        strm.run()
        strm.join()
        end_time = counter
