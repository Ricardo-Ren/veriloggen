...
def mkLed():
    ...
    for i in range(core_num):
        a_vec = strm.source_parallel('a'+str(i+1), par=para)  # 32 * 4
        b_vec = strm.source_parallel('b'+str(i+1), par=para)  # 32 * 4
        c_vec = a_vec + b_vec
        ...
        valid = sum_valid
        sum_list.append(sum)
        # new_variable_name = "sum"+str(i)
        # exec(new_variable_name + " = sum")

    # a_vec1 = strm.source_parallel('a1', par=para)  # 32 * 4
    # b_vec1 = strm.source_parallel('b1', par=para)  # 32 * 4
    ...
    # # core 2
    # a_vec2 = strm.source_parallel('a2', par=para)  # 32 * 4
    # b_vec2 = strm.source_parallel('b2', par=para)  # 32 * 4
    ...
    # a_vec3 = strm.source_parallel('a3', par=para)  # 32 * 4
    # b_vec3 = strm.source_parallel('b3', par=para)  # 32 * 4
    # c_vec3 = a_vec3 + b_vec3
    # #c_vec = a_vec * b_vec + bias # parallel multiplication (4) with 1 scala value
    # args3 = c_vec1.par_var_list
    # c3 = strm.AddN(*args3) # c = c_vec[0] + c_vec[1] + c_vec[2] + c_vec[3]
    # sum3, sum_valid3 = strm.ReduceAddValid(c3, size)

    # a_vec4 = strm.source_parallel('a4', par=para)  # 32 * 4
    # b_vec4 = strm.source_parallel('b4', par=para)  # 32 * 4
    # c_vec4 = a_vec4 + b_vec4
    # #c_vec = a_vec * b_vec + bias # parallel multiplication (4) with 1 scala value
    # args4 = c_vec4.par_var_list
    # c4 = strm.AddN(*args4) # c = c_vec[0] + c_vec[1] + c_vec[2] + c_vec[3]
    # sum4, sum_valid4 = strm.ReduceAddValid(c4, size)
    
    sum = strm.AddN(*sum_list)
    # sum = strm.AddN(*[sum1, sum2, sum3, sum4])
    strm.sink(sum, 'sum', when=valid, when_name='valid')
    # Timer
    ...
    def comp_stream(size, offset):
        start_time = counter
        size = size//core_num
        # strm.schedule(core_num, )
        strm.set_parameter('size', size//para)
        # strm.set_source_parallel('a1', ram_a, offset+(size//(2*para))*i, size//para)
        # strm.set_source_parallel('a2', ram_a, offset+(size//(2*para))*i, size//para)
        # strm.set_source_parallel('a3', ram_a, offset+(size//(2*para))*i, size//para)

        for i in range(core_num):
            strm.set_source_parallel('a'+str(i+1), ram_a, offset+(size//(2*para))*i, size//para)
            strm.set_source_parallel('a'+str(i+1), ram_b, offset+(size//(2*para))*i, size//para)
        
        # strm.set_source_parallel('a1', ram_a, offset, size//para)
        # strm.set_source_parallel('a2', ram_a, offset+(size//(2*para)), size//para)
        # strm.set_source_parallel('a3', ram_a, offset+(size//(2*para))*2, size//para)
        # strm.set_source_parallel('a4', ram_a, offset+(size//(2*para))*3, size//para)
        # strm.set_source_parallel_pattern('a2', ram_a, offset+para, (para, size))
        print("ssss:",size//para)
        # strm.set_source_parallel('b1', ram_b, offset, size//para)
        # strm.set_source_parallel('b2', ram_b, offset+(size//(2*para)), size//para)
        # strm.set_source_parallel('b3', ram_b, offset+(size//(2*para))*2, size//para)
        # strm.set_source_parallel('b4', ram_b, offset+(size//(2*para))*3, size//para)
    ...
    def comp(size):
        # stream
        offset = 0
        # myaxi.dma_read_packed(ram_a1, offset, 0, size//2)
        # myaxi.dma_read_packed(ram_a2, offset, size//2, size//2)
        # myaxi.dma_read_packed(ram_b1, offset, 512, size//2)
        # myaxi.dma_read_packed(ram_b2, offset, 512+size//2, size//2)
