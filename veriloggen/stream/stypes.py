...
class _par__Variable:
    def __init__(self, par = 1, data=None, width=32, point=0, signed=True, Variable_list=[]) -> None:
        # Variable_list = list()
        self.Variable_list = Variable_list
        self.par = par
        if Variable_list == []:
            for i in range(par):
                self.Variable_list.append(_Variable(data, width, point, signed))
        
    def __add__(self, r):
        ret = _par__Variable(par = self.par, Variable_list = self.Variable_list)
        for i in range (self.par):
            ret.Variable_list[i] = self.Variable_list[i]+r.Variable_list[i]
        return ret
